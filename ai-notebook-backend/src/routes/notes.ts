...
// 使用持久化存储
export const getNotes = () => NoteStorage.getAll();
...
router.post('/', authenticateToken, async (req, res, next) => {
  try {
    ...
    // 验证必填字段 - 标题或内容至少有一个
    if (!title && !content) {
      throw createError('标题或内容不能同时为空', 400);
    }
...
router.put('/:id', authenticateToken, async (req, res, next) => {
  try {
    ...
    const updatedNote = NoteStorage.update(noteId, updates);
...
router.post('/:id/favorite', authenticateToken, async (req, res, next) => {
  try {
    ...
    const updatedNote = NoteStorage.update(noteId, {
