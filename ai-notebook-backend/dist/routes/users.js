...
function getNotebooksData() {
    return storage_1.NotebookStorage.getAll();
}
// 获取笔记数据的函数
function getNotesData() {
    return storage_1.NoteStorage.getAll();
}
// 获取用户统计信息
router.get('/stats', async (req, res, next) => {
    try {
        ...
        const userNotebooks = allNotebooks.filter((nb) => nb.user_id === userId);
        const userNotes = allNotes.filter((note) => note.user_id === userId && !note.is_deleted);
        // 更新每个笔记本的笔记数量
        userNotebooks.forEach((notebook) => {
            const notebookNotes = userNotes.filter((note) => note.notebook_id === notebook.id);
            notebook.note_count = notebookNotes.length;
        });
        // 计算统计数据
        const totalNotes = userNotes.length;
        const totalNotebooks = userNotebooks.length;
        const totalWords = userNotes.reduce((sum, note) => {
            // 简单的字数统计（去除HTML标签）
        ...
        const notesThisMonth = userNotes.filter((note) => {
