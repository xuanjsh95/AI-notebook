...
class ChatService {
    constructor() {
        this.apiConfigStorage = new apiConfigStorage_1.ApiConfigStorage();
    }
    // 发送消息
    ...
    getProviderFromUrl(baseUrl) {
        ...
        if (baseUrl.includes('huggingface.co'))
            return 'Hugging Face';
        if (baseUrl.includes('together.ai'))
            return 'Together AI';
        if (baseUrl.includes('replicate.com'))
            return 'Replicate';
        // 尝试从域名提取
        try {
